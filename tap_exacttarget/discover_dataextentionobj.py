...
def discover_dao_streams(client: Client):
    ...
    while has_more_data:
        ...
        if "Error" in status:
            raise MarketingCloudError(f"Request failed with status: {status}, Unable to discover Streams")

        results = response["Results"]
        LOGGER.info("discovered %s DataExtentions", len(results))
        ...
        for item in results:
            ...
            base_class = DataExtentionObjectFt if not repl_keys else DataExtentionObjectInc
            # creates duplicate class objects for each stream
            stream_class = type(
                class_name,
                (base_class,),
                {

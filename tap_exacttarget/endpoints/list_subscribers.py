...
from util import partition_all, sudsobj_to_dict
...
class ListSubscriberDataAccessObject(DataAccessObject):
    ...
    def _get_all_subscribers_list(self):
        ...
        if len(lists) != 1:
            msg = "Found {} all subscriber lists, expected one!".format(len(lists))
    ...
    def sync_data(self):
        ...
        subscriber_dao = SubscriberDataAccessObject(
            self.config, self.state, self.auth_stub, self.subscriber_catalog
        )

...
class DataExtensionDataAccessObject(DataAccessObject):
                ...
                "schema": {
                    ...
                    "properties": {
                        "_CustomObjectKey": {
                            "type": ["null", "string"],
                            "description": (
                                "Hidden auto-incrementing primary "
    ...
    def _get_fields(self, extensions):  # pylint: disable=too-many-branches
        ...
        for field in result:
            ...
            if field.get("IsPrimaryKey"):
                ...
                is_primary_key = True
                to_return = _merge_in(
                    to_return, [extension_id, "key_properties"], field_name
                )
            ...
            if is_primary_key:
                for mdata in to_return[extension_id]["metadata"]:
                    if not mdata.get("breadcrumb"):
                        mdata.get("metadata").get("table-key-properties").append(
                            field_name
                        )
            ...
            if is_replication_key:
                for mdata in to_return[extension_id]["metadata"]:
                    if not mdata.get("breadcrumb"):
                        ...
                        mdata.get("metadata").get("valid-replication-keys").append(
                            field_name
                        )
    ...
    def filter_keys_and_parse(self, obj):
        ...
        for k, v in to_return.items():
            ...
            if v is None:
            ...
            elif "boolean" in field_schema.get("type") and isinstance(
                to_return[k], str
            ):
                ...
                if str(to_return[k]).lower() in [1, "1", "y", "yes", "true"]:
                    ...
                    LOGGER.warn(
                        "Could not infer boolean value from {}".format(to_return[k])
                    )
                    to_return[k] = None
    ...
    def _replicate(
        ...
        if partial:
            LOGGER.info("Fetching {} from {} to {}".format(table, start, end))
        ...
        if partial:
            cursor.search_filter = get_date_page(replication_key, start, unit)

        result = request_from_cursor(
            "DataExtensionObject", cursor, batch_size=self.batch_size
        )
        ...
        for row in result:
            ...
            self.state = incorporate(
                self.state, table, replication_key, row.get(replication_key)
            )

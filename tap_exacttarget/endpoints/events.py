    ...
    def sync_data(self):
        ...
        for event_name, selector in endpoints.items():
            ...
            while before_now(start):
                LOGGER.info("Fetching {} from {} to {}".format(event_name, start, end))
                ...
                for event in stream:
                    ...
                    self.state = incorporate(
                        self.state, event_name, "EventDate", event.get("EventDate")
                    )

                    if event.get("SubscriberKey") is None:
                        LOGGER.info(
                            "SubscriberKey is NULL so ignoring {} record with SendID: {} and EventDate: {}".format(
                                event_name, event.get("SendID"), event.get("EventDate")
                            )
                        )
                        continue
